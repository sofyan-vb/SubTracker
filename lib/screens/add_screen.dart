...
class _AddScreenState extends State<AddScreen> {
      ...
      builder: (context, _, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(tr('Tambah Langganan', 'Add subscription', 'Agregar suscripción'), style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            centerTitle: true,
          ),
            ...
            child: FloatingActionButton.extended(
              ...
              onPressed: () async { 
                ...
                if (_nameCtrl.text.isNotEmpty && _priceCtrl.text.isNotEmpty) {
                  ...
                  if (mounted) {
                    ToastUtils.show(context, tr('Berhasil ditambahkan', 'Successfully added', 'Agregado exitosamente'));
              ...
              label: Text(tr('Simpan Langganan', 'Save Subscription', 'Guardar suscripción'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ),
          ),
        );
      }
  ...
  Widget _buildModernCategoryIcons(List<String> categories, Color cardBg, Color textColor, bool isDark) {
    ...
    final validCategories = CategoryUtils.getAllCategories(languageNotifier.value == 'ID');
    List<Map<String, dynamic>> allAppsWithCategory = [];
    for (String cat in validCategories) {
      if (appExamples.containsKey(cat)) {
        for (var app in appExamples[cat]!) {
